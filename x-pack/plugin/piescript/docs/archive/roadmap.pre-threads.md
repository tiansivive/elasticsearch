...
| `ExchangeVal` — serializable exchange descriptor (ID + column names + buffer size) | :white_check_mark: |
| `Exchange.open` / `Exchange.sink` / `Exchange.connect` — exchange setup (D-054) | :white_check_mark: |
| `Exchange.addPage` / `Exchange.poll` / `Exchange.finish` — exchange operations (D-054) | :white_check_mark: |
| Unit + integration tests, debug scripts | :construction: debug scripts done, NbE unit tests done; integration tests :memo: |
| Documentation (D-054 decision record, architecture, roadmap) | :white_check_mark: |

**Additional progress** (2026-04-07):
- Action namespace changed to `cluster:compute/piescript` (was `indices:data/read/piescript`),
  send action → `internal:compute/piescript/send`. See D-055.
- Nested record types from field caps: OBJECT fields recursively build `RecordType` in
  `IndexResolutionPrePass`, so `use "idx" as idx` gets nested row types.
- `ESQL.keep`/`ESQL.drop` NbE fix: dotted field paths (e.g., `host.name`) now work correctly
  via `Symbol` values instead of record keys.
- `BooleanBlock` support in `Shard.stream` for boolean fields.
- `Shard.write` uses INDEX (upsert) semantics (`autoGeneratedTimestamp = -1`).
...
| ~~`ActionType` name scope review~~ | ~~Medium~~ | **Resolved** (D-055). Action namespace changed to `cluster:compute/piescript` (cluster-level auth). Send action → `internal:compute/piescript/send`. `CompositeIndicesRequest` removed from `PiescriptRequest`. Piescript removed from `RBACEngine` allowlist. |
