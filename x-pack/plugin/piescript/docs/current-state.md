...
> exists right now."
>
> **Last updated**: 2026-04-13 (Recursion Phase 1 close-out: implicit recursion + `loop`/`repeat` implemented, integration tests and docs reconciled.)
...
`ESQL.stats`/`ESQL.statsBy` + aggregate builtins) is complete. Recursion Phase 1
(implicit recursion + fused `loop`/`repeat`) is complete.**
...
**Phase 1e (Pattern Matching) is complete. Recursion + loop/repeat are implemented.** The execution
...
| ESQL query compilation (Block F) | `query ESQL.from idx \|> ESQL.where (fn r -> r.age > 18) \|> ESQL.limit 10;` compiles piescript combinators to ESQL via NbE-style partial evaluation. `ESQL r` type constructor, `query expr ;` syntax (replaces old backtick ESQL). Closures partially evaluated with `Symbol("")` row — projections produce `Symbol(field)`, primops compile to `Symbol("(left OP right)")`. ESQL string built incrementally during evaluation. Combinators: `ESQL.from`/`where`/`eval`/`keep`/`drop`/`limit`/`sort`/`sortDesc`/`rename`/`explain`/`stats`/`statsBy`/`count`/`countOf`/`avg`/`sum`/`max`/`min`/`bucket`/`top`/`values`. See D-052, D-053. |
| MV-returning ESQL aggregates | `ESQL.top r.field N "order"` and `ESQL.values r.field` return `List a` instead of scalar. Type-driven materialization: evaluator `force`s the elaborated row type from `CoreQueryExec.type()`, identifies `List`-typed columns, and `EsqlValueConverter` materializes those as `ListVal` (all elements preserved). Other columns use scalar conversion (first element). Unblocks the risk score query pattern: ESQL handles TOP-N, piescript handles user-defined aggregates over the resulting lists. |
| Nested record types from field caps | `IndexResolutionPrePass` recursively builds `RecordType` for OBJECT fields. `use "idx" as idx` now gets nested row types (e.g., `{ host: { name: Keyword, ip: Keyword }, ... }`). |
| Block G streaming: Exchange builtins | `ExchangeVal` (serializable descriptor), `Exchange.open`/`sink`/`connect`/`addPage`/`poll`/`finish` backed by ESQL's `ExchangeService`. `Shard.stream` converts `DocRef` batches to columnar `Page` via Block builders. `Page.toList`/`Page.count` for materialization. `BooleanBlock` support for boolean fields. |
| `Shard.write` INDEX semantics | `Shard.write` uses INDEX (upsert) semantics (`autoGeneratedTimestamp = -1`). Existing documents with the same `_id` are overwritten. |
| `force` function in evaluator | `ElaborationState.force` threaded to evaluator via `EvalDependencies` for type-driven materialization. Used by `EsqlValueConverter` to identify `List`-typed columns in MV aggregates. |
| `ESQL.keep`/`drop` dotted paths | NbE fix: `ESQL.keep`/`ESQL.drop` now use dotted field paths from `Symbol` values (e.g., `host.name`) instead of record keys. Correctly handles nested fields in ESQL compilation. |
...
| Recursion + loop/repeat | Implicit recursion for all let-bindings via tying-the-knot evaluation (`CoreLet` backpatch). Guarded recursion check rejects unguarded self-reference (`let x = x + 1 in x`) outside lambda bodies. `loop`/`repeat` syntax elaborates to `CoreLoop`/`CoreRepeat` using `Repeat a` as loop-local control type; `repeat` outside a loop is an elaboration error. |
...
| Transport layer (Block C) | `PiescriptSendAction` (`internal:compute/piescript/send`), `PiescriptSendRequest` (channelId + serialized value), `TransportPiescriptSendAction` (handler with inbox and regular channel dispatch). `ChannelRegistry` is a singleton per node, shared across transport actions via Guice injection. |
...
| Integration tests | `PiescriptIT` + `PiescriptMultiNodeIT` cover query type-checking, eager evaluation, expression evaluation, pattern matching, recursion (`factorial`/`fibonacci`), `loop`/`repeat`, topology, list utilities, error handling, cross-node execution, `use` declarations, shard data access, shard writes, `Index.bulk`, global checkpoints, and non-serializable value response/wire rejection |
...
| Core IR (Phase 1b + D-035 + Phase 2 + Block A + Block C + Phase 1e + recursion/loop additions) | `CoreExpr` sealed hierarchy in `piescript.core`: `CoreVar`, `CoreFree`, `CoreLit`, `CoreLam`, `CoreApp`, `CoreLet`, `CoreRecord`, `CoreProject`, `CoreUpdate`, `CorePrimOp`, `CoreTypeAbs`, `CoreTypeApp`, `CoreQuery`, `CoreSpawn`, `CoreWhen`, `CoreSend`, `CoreMatch`, `CoreLoop`, `CoreRepeat` — extends `Node<CoreExpr>` with `MonoType` on every node. `CoreTypeAbs` and `CoreTypeApp` are unary System F nodes. `CoreFree` is a module-level free variable (built-ins). `CoreQuery` carries an ESQL query string and its resolved list type. `CoreSpawn` wraps a `@Nullable` body expression (type `Channel bodyType`); null body represents bare `spawn!` channel creation. `CoreSend` takes a channel expression and a value expression (type `Null`). `CoreWhen` carries a list of `WhenBinding(CoreExpr channel, @Nullable String debugName)` and a body. `CoreMatch` carries a scrutinee and a list of `Alternative(Pattern, CoreExpr)` arms. `CoreLoop` carries init + arms; `CoreRepeat` carries the next state expression. |
...
| Runtime values (Phase 1c + Phase 2 + Block A + Block B + Block C + Block D + recursion/loop additions) | `Value` sealed interface in `piescript.eval`: `IntegerVal`, `LongVal`, `DoubleVal`, `KeywordVal(String)`, `BooleanVal`, `NullVal`, `RecordVal`, `ListVal(List<Value>)`, `ClosureVal`, `BuiltinVal(name, arity, partialArgs)`, `ChannelVal(nodeId, channelId)`, `IndexVal(name, uuid, fieldTypes)`, `SearcherVal(SearcherState)`, `DocRefVal(docId, SearcherState)`, `RepeatVal(newState)`. `ListVal` is the eagerly materialized list representation (renamed from `StreamVal` in Block B — D-043). `BuiltinVal` supports curried partial application. `ChannelVal` is a serializable channel reference (Block C — D-045). `IndexVal` is serializable (carries index metadata). `SearcherVal` and `DocRefVal` are **non-serializable** — they hold node-local Lucene state. `RepeatVal` is loop-local control and is rejected at wire serialization boundaries. |
...
| Evaluator tests (Phase 1c + Phase 2 + Block A + math builtins + recursion/loop additions) | `EvaluatorTests.java` covers recursive factorial/fibonacci and loop/repeat evaluation in addition to existing Phase 1c/Phase 2 suites (`spawn`/`when`, math builtins, async and deterministic execution modes). |
