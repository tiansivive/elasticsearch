...
- `Shard.write : ∀r. Writer r → Keyword → r → WriteResult` — writes a single document to the
  ...
  no ingest, no routing. Uses INDEX (upsert) semantics — writing the same `_id` twice updates
  the doc rather than failing (`autoGeneratedTimestamp = -1` signals a user-provided ID to the
  engine). Future: allow the user to specify CREATE vs INDEX when error handling (sum types /
  result types) is available.
...
## D-054: Block G — Streaming Data Access and Exchange Design

**Phase**: Block G | **Status**: accepted

**Context**: Piescript's read path (`Shard.read`) works doc-by-doc, materializing everything into
`List<Value>` on the coordinator. This does not scale. ESQL's compute engine provides columnar
`Page` objects and the `ExchangeService` for backpressure-controlled streaming between nodes. Block G
integrates these into piescript as composable builtins.

**Decision**: Introduce columnar `Page r` values, an `Exchange r` descriptor for setting up
streaming pipelines, and callback-based consumption.

### New types

| Type | Kind | Serializable | Purpose |
|------|------|-------------|---------|
| `Page r` | `Row → Type` | No (node-local) | Columnar data from compute engine |
| `Sink r` | `Row → Type` | No (node-local) | Exchange sink handle |
| `Source r` | `Row → Type` | No (node-local) | Exchange source handle |
| `Exchange r` | `Row → Type` | **Yes** | Exchange descriptor (ID + column names + buffer size) |
...
1. **`Exchange r` is serializable, `Sink r` / `Source r` are not.** The exchange descriptor
   carries an exchange ID, column names, and buffer size — pure data that can travel in closures
   across nodes. The actual sink/source infrastructure is instantiated locally on each node by
   calling `Exchange.sink` or `Exchange.connect`. This keeps the API uniform: the caller does not
   think about local vs remote — the implementation routes via `ExchangeService` based on
   whether the exchange was opened on the local or a remote node.

2. **`Exchange.poll` is callback-based.** `poll : Source r → (Page r → Null) → Channel Null`
   invokes the callback for each page as it arrives and completes the `Channel Null` when the
   source is exhausted. This avoids the need for union types (`Page r | Null`) which piescript
   does not have, and avoids creating a new channel per page. Backpressure is handled internally
   via `ExchangeSource.waitForReading()`.

3. **Column names are explicit at `Exchange.open` time.** The `List Keyword` parameter provides
   the field names that map block indices to record fields. Both `Exchange.sink` and
   `Exchange.connect` derive column names from the descriptor. This is necessary because compute
   engine `Page` objects carry only `Block[]` with no field name metadata (ESQL uses a separate
   `Layout` metadata track through its operator pipeline; piescript uses the exchange descriptor
   instead).

4. **`Shard.stream` uses compute Block builders directly.** Rather than the full `BlockLoader`
   infrastructure (which requires `BlockLoaderContext`, `DelegatingBlockLoaderFactory`, circuit
   breaker integration, etc.), `Shard.stream` reads doc values directly into compute engine
   Block builders (`LongBlock.Builder`, `DoubleBlock.Builder`, `BytesRefBlock.Builder`). This
   produces real compute `Page` objects compatible with the Exchange system. Upgrade to
   `BlockLoader` is future work for optimized column-at-a-time reads.

5. **`ExchangeService` manages exchange infrastructure.** Piescript uses ESQL's `ExchangeService`
   (injected via Guice from the ESQL plugin) for exchange handler registration and lifecycle
   management. `Exchange.sink` calls `ExchangeService.getOrCreateSinkHandler` to register a
   sink handler. `Exchange.connect` creates an `ExchangeSourceHandler` and attaches a
   `RemoteSink` via `ExchangeService.newRemoteSink`. For same-node exchanges, the remote sink
   uses `TransportService.getLocalNodeConnection()` — the transport layer handles local
   routing transparently. Remote (cross-node) exchanges use the same code path with a remote
   connection instead.

### Known issues and future improvements

- **Column name mismatch is not caught statically.** The `List Keyword` parameter to
  `Exchange.open` is a runtime value — the type checker cannot verify that the string list
  matches the fields of the row type `r`. A runtime check on `Exchange.addPage` (comparing
  `PageVal` column names with exchange column names) provides a safety net. Long-term fix:
  type-level string lists or reflection of `r`'s field names at elaboration time, allowing
  column names to be derived from the type rather than provided explicitly.

- **`Shard.stream` column name consistency.** Same issue: `Shard.stream` derives column names
  from the mapper service at runtime. If the user passes different column names to
  `Exchange.open` than what `Shard.stream` produces, the mismatch is caught at runtime (on
  `addPage`) but not at compile time.

- **`BlockFactory` uses `NoopCircuitBreaker`.** The current `Shard.stream` implementation
  creates blocks without circuit breaker accounting. Production use should integrate with ES
  circuit breakers to prevent OOM on large page batches.

**Rationale**: This design separates the exchange *description* (serializable, type-safe) from
the exchange *infrastructure* (node-local, backed by ExchangeService). The row type parameter `r`
enforces schema consistency across `Exchange.open`, `Exchange.sink`, `Exchange.connect`,
`Exchange.addPage`, and `Exchange.poll` at elaboration time. The callback-based `poll` fits
piescript's existing async patterns (CPS via `ActionListener`, `Channel` for completion).

**Alternatives considered**:
- Separate local (`Exchange.create`) vs remote (`Exchange.openSink`/`connectSource`) APIs:
  rejected — leaks topology into the user API. The `Exchange r` descriptor abstracts over both.
- `Exchange.poll` returning `Channel (Page r)` per page: rejected — creates excessive channels,
  wasteful for streaming.
- `Exchange.poll` returning `Page r | Null`: rejected — piescript has no union/sum types.
- `Exchange.poll` with `done` check + blocking poll: rejected — error-prone pattern, relies on
